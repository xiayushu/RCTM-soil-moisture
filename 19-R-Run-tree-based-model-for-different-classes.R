...
all$Month <- as.numeric(tstrsplit(all$Date, "/")[[1]])

### Define class (the subset) for building tree-based model
#Depth-based model
all[all$Depth == 51,]$Depth <- 50
all[all$Depth == 102,]$Depth <- 100
all <- all[which(all$Depth == 50),]
#LULC-based model
#all <- all[which(all$landcover == "Grassland"),]
#Month-based model
#all <- all[which(all$Month == 1),] 


### Random division of calibration and validation sites
